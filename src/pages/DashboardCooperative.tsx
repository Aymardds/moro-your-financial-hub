...
import { Users, Building2, TrendingUp, FileText, Shield, MapPin } from 'lucide-react';
...
export default function DashboardCooperative() {
  ...
  const [applications, setApplications] = useState<any[]>([]); // New state
  ...
  const [stats, setStats] = useState({
    ...
    activeLoans: 0,
    totalLoans: 0,
    pendingApplications: 0, // New stat
  });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Récupérer les demandes de financement (Workflow 3-Tier)
      // Note: RLS ensures we only see our members' applications
  ...
  const handleValidation = async (id: string, action: 'approve' | 'reject') => {
    try {
      const newStatus = action === 'approve' ? 'approved_by_coop' : 'rejected_by_coop';

      // If approved, it automatically goes to admin?
      // "Tier 2 (Moro Admin): After cooperative approval, Moro Admin reviews..."
      // So status should be 'submitted_to_admin' immediately?
      // Or 'approved_by_coop' and then a trigger/process moves it?
      // Let's set it to 'submitted_to_admin' directly to signal it's ready for admin,
      // OR set 'approved_by_coop' and have the Admin fetch those.
      // 008 SQL comment says: "pending -> submitted_to_coop -> approved_by_coop -> submitted_to_admin ..."
      // This implies explicit steps. 
      // If I set 'approved_by_coop', does it sit there?
      // I'll set 'approved_by_coop' AND update logic in Admin to look for 'approved_by_coop' OR 'submitted_to_admin'.
      // Actually, for simplicity, I will set 'submitted_to_admin' if approved.
      // Wait, "approved_by_coop" is a distinct state.
      // I'll use 'approved_by_coop'. The Admin Dashboard should pick these up.
